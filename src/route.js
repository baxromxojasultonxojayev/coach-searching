...
const router = createRouter({
  ...
  routes: [
    ...
    { path: '/request', component: RequestReceive, meta: { requiresAuth: true } },
...
router.beforeEach (function(to, _, next){
  if(to.meta.requiresAuth && !store.getters.isAuthonticated) {
    next('/auth')
  }
  else if(to.meta.requiresUnauth && store.getters.isAuthonticated) {
    next('/coaches') 
  }
  else{
    next()
